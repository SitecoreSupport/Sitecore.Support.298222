...
namespace Sitecore.Support.XA.Foundation.Search.Services
{
  public class SearchService : ISearchService
  {
    ...
    public IQueryable<ContentPage> GetQuery(string query, string scope, string language, Coordinates center, string site, string itemid, out string indexName)
    {
      ...
      searchIndex.CreateSearchContext(SearchSecurityOptions.Default);
      ...
      IQueryable<ContentPage> source = default(IQueryable<ContentPage>);
      using (new SiteContextSwitcher(SiteContextFactory.GetSiteContext("shell")))
      {
        source = LinqHelper.CreateQuery<ContentPage>(searchIndex.CreateSearchContext(SearchSecurityOptions.Default), models);
      }
    ...
    protected virtual Expression<Func<ContentPage, bool>> PageOrMediaPredicate(string siteName)
    {
      ...
      if (settingsItem != null)
      {
        ...
        {
          foreach (string item in from i in multilistField.TargetIDs
                                  select i.ToSearchID())
          {
            expression = expression.Or((ContentPage i) => i.RawPath == item && i.IsSearchable);
          }
        }
        ...
        {
          foreach (string item2 in from i in multilistField2.GetItems()
                                   select i.ID.ToSearchID())
          {
            expression = expression.Or((ContentPage i) => i.RawPath == item2);
          }
          return expression;
        }
      }
      ...
      if (settingsItem != null)
      {
        ...
        if (multilistField != null)
        {
          foreach (string item in from i in multilistField.TargetIDs
                                  select i.ToSearchID())
          {
            expression.Or((ContentPage i) => i.RawPath == item && i.IsPoi);
          }
          return expression;
        }
      }
    ...
    protected virtual Expression<Func<ContentPage, bool>> ContentPredicate(string content)
    {
      ...
      foreach (string item in content.Split(Array.Empty<char>()).TrimAndRemoveEmpty())
      {
        string t = item;
        expression = expression.And((ContentPage i) => i.AggregatedContent.Equals(t));
      }
      return expression;
    }
